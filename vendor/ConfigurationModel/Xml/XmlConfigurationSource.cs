...
namespace Microsoft.Framework.ConfigurationModel
{
    ...
    public class XmlConfigurationSource : ConfigurationSource
    {
        ...
        //    : this(path // , xmlDocumentDecryptor
        //          , optional: false)
        //{
        //}
            ...
            , bool optional)
        {
            ...
            _xmlDocumentDecryptor = XmlDocumentDecryptor.Instance;
        }
            ...
            using (var reader = _xmlDocumentDecryptor.CreateDecryptingXmlReader(stream, readerSettings))
            {
                ...
                while (reader.Read())
                {
                    switch (reader.NodeType)
                    {
                        ...
                        case XmlNodeType.Text:
                            {
                                ...
                                if (data.ContainsKey(key))
                                {
                                    throw new FormatException(
                                        ...
                                        String.Format("FormatError_KeyIsDuplicated {0}", key,
                                        GetLineInfo(reader)));
                                }
                        ...
                        default:
                            throw new FormatException(
                                    // Resources.FormatError_UnsupportedNodeType
                                    String.Format("FormatError_UnsupportedNodeType {0}, {1}", reader.NodeType,
                                GetLineInfo(reader)));
                    }
        ...
        private static string GetLineInfo(XmlReader reader)
        {
            ...
            return lineInfo == null ?  string.Empty :
                // Resources.FormatMsg_LineInfo
                String.Format("FormatMsg_LineInfo {0} {1}", lineInfo.LineNumber, lineInfo.LinePosition);
        }
            ...
            for (int i = 0; i < reader.AttributeCount; i++)
            {
                ...
                if (!string.IsNullOrEmpty(reader.NamespaceURI))
                {
                    throw new FormatException(
                        ...
                        String.Format("FormatError_NamespaceIsNotSupported {0}", (GetLineInfo(reader)) ));
                }
            ...
            IDictionary<string, string> data, XmlWriter writer)
        {
            ...
            if (data.ContainsKey(key))
            {
                throw new FormatException(
                    // Resources.FormatError_KeyIsDuplicated(
                    String.Format("FormatError_KeyIsDuplicated {0} {1}", key, GetLineInfo(reader)));
            }
