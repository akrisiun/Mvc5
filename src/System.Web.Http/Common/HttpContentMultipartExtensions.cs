...
namespace System.Net.Http
{
    ...
    public static class HttpContentMultipartExtensions
    {
        ...
        [SuppressMessage("Microsoft.Design", "CA1006:DoNotNestGenericTypesInMemberSignatures", Justification = "Nesting of generic types is required with Task<T>")]
        public static Task<IEnumerable<HttpContent>> ReadAsMultipartAsync(this HttpContent content,
               MultipartStreamProvider // IMultipartStreamProvider 
               streamProvider)
        {
               ...
               streamProvider, int bufferSize)
        {
            ...
            {
                var taskCompletionSource = new TaskCompletionSource<IEnumerable<HttpContent>>();
                var parser = new MimeMultipartBodyPartParser(content, streamProvider);
                byte[] data = new byte[bufferSize];
                var context = new MultipartAsyncContext(stream, taskCompletionSource, parser, data);
        ...
        private static void MultipartReadAsync(MultipartAsyncContext context)
        {
            ...
            {
                Exception exception = (result != null && result.CompletedSynchronously) ? e : 
                    new IOException(HttpResources.ReadAsMimeMultipartErrorReading, e);
        ...
        private static void MultipartReadAsyncComplete(IAsyncResult result)
        {
            ...
            {
                context.TaskCompletionSource.TrySetException(new IOException(
                    HttpResources.ReadAsMimeMultipartErrorReading, e));
            }
        ...
        private static void MultipartWriteSegmentAsync(MultipartAsyncContext context)
        {
            ...
            catch (Exception e)
            {
                ...
                context.TaskCompletionSource.TrySetException(new IOException(
                    HttpResources.ReadAsMimeMultipartErrorWriting, e));
            }
        }
