...
namespace System.Net.Http
{
    ...
    /// </summary>
    // internal 
    class MimeBodyPart : IDisposable
    {
        ...
        public Stream GetOutputStream()
        {
            if (_outputStream == null)
            {
                ...
                {
                    throw Error.InvalidOperation(e, HttpResources.ReadAsMimeMultipartStreamProviderException, _streamProvider.GetType().Name);
                }

                if (_outputStream == null)
                {
                    throw Error.InvalidOperation(HttpResources.ReadAsMimeMultipartStreamProviderNull, _streamProvider.GetType().Name, _streamType.Name);
                }

                if (!_outputStream.CanWrite)
                {
                    throw Error.InvalidOperation(HttpResources.ReadAsMimeMultipartStreamProviderReadOnly, _streamProvider.GetType().Name, _streamType.Name);
                }
